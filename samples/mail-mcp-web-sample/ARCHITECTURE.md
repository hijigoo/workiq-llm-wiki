# 아키텍처 문서 — Mail MCP Web Sample

사용자가 웹 UI에서 자연어 또는 도구 호출로 요청하면, **Work IQ Mail** MCP 서버
(`mcp_MailTools`)를 통해 Microsoft Graph 메일(읽기/검색/작성/회신/전송/관리) 작업을 수행하고
...
- **핵심 설계 포인트**: **서로 다른 두 개의 신원(identity)** 을 사용
  1. Mail MCP 호출 → **로그인한 사용자**의 위임(delegated) 토큰
  2. Azure OpenAI 호출 → **서버 실행 신원**(`DefaultAzureCredential`)의 토큰

```mermaid
graph LR
  U["사용자<br/>(브라우저)"]
  subgraph APP["Node.js / Express 앱 (localhost:3001)"]
  ...
  TEAMS["Work IQ Mail MCP<br/>agent365.svc.cloud.microsoft"]
...
- Mail MCP 스코프를 **`${MCP_SERVER_URL}/.default`** 로 구성(커스텀 API 리소스는 `.default`만 노출).
...
> **중요**: LLM은 **어떤 도구를 어떤 인자로 부를지**만 결정합니다. 실제 메일 데이터 접근은
...
graph TB
  subgraph ID1["신원 1 · 로그인한 사용자 (위임)"]
    ...
    T1["액세스 토큰<br/>aud: Agent Tools(agent365)<br/>scp: McpServers.Mail.All"]
    MSAL --> T1 --> TEAMS2["Mail MCP<br/>mcp_MailTools"]
  end
...
| | 신원 1 (Mail MCP) | 신원 2 (Azure OpenAI) |
|---|---|---|
| 주체 | 로그인한 최종 사용자 | 서버 프로세스 신원 |
| 획득 방식 | MSAL 브라우저 로그인(위임) | `DefaultAzureCredential`(로컬=`az login`, 배포=Managed Identity) |
| 스코프 | `<MCP_SERVER_URL>/.default` → `McpServers.Mail.All` | `https://cognitiveservices.azure.com/.default` |
...
| 왜 분리? | 메일 데이터는 사용자 권한으로만 접근 | LLM 추론은 사용자 신원과 무관, 키 대신 RBAC로 통제 |

> **앱 재사용 노트:** Teams·Mail MCP 서버는 **동일한 Agent Tools 리소스 앱**(`ea9ffc3e-…`,
> `https://agent365.svc.cloud.microsoft`)에 속하므로, 하나의 Entra 앱을 두 서버에 재사용할 수
> 있습니다(토큰 audience 동일). 이 샘플도 기존 `teams-mcp-web-sample` 앱
> (`72848081-…499c`)을 그대로 재사용했습니다. 단, MCP는 `.default` 로 토큰을 요청하므로
> **각 서버의 위임 스코프(`McpServers.Teams.All` / `McpServers.Mail.All`)를 각각 동의**해야 하며,
> 동의되지 않은 서버는 `Scope ... is not present` 로 거부합니다.
...
- **최소 권한**: 메일 접근은 사용자 위임 토큰 범위로 제한 → LLM/서버가 사용자 권한을 초과 불가.
