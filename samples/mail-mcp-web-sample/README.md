...
사용자가 웹에서 요청을 입력하면 **Work IQ Mail** MCP 서버(`mcp_MailTools`)를 통해
응답을 받는 샘플 웹 앱입니다. Microsoft Graph 메일(읽기/검색/작성/회신/전송/관리) 작업만 다룹니다.

```
브라우저(채팅 UI) → Express 서버 → (Entra OAuth 토큰) → Mail MCP (streamable HTTP)
```
...
- 브라우저에서 **http://localhost:3001** 접속 → 우측 상단 **Microsoft 로그인**.
- 로그인 후 좌측에 Mail MCP 도구 목록이 뜨면 정상(상태등 초록).
...
대상 MCP 엔드포인트:
```
https://agent365.svc.cloud.microsoft/agents/tenants/00000000-0000-0000-0000-000000000000/servers/mcp_MailTools
```
...
> 이 샘플에서는 새 앱을 만들지 않고 **기존 `teams-mcp-web-sample` 앱을 재사용**했습니다
> (Client ID `11111111-1111-1111-1111-111111111111`). Teams·Mail MCP는 같은 "Agent Tools"
> 리소스라 audience가 동일하지만, **Mail은 `McpServers.Mail.All` 위임 권한을 별도로 추가·동의**해야
> 합니다. 재사용 시 아래 1(리디렉션 URI 추가)·3·4번만 수행하면 됩니다.

1. [Entra 관리 센터](https://entra.microsoft.com) → **App registrations**
   - 새 앱: **New registration**, Redirect URI **Web** = `http://localhost:3001/auth/callback`
   - 기존 앱 재사용: **Authentication → Redirect URIs** 에 `http://localhost:3001/auth/callback` 추가.
2. **Application (client) ID** 를 `.env`의 `CLIENT_ID` 에 설정.
3. **API permissions → Add a permission → APIs my organization uses** 에서
   `mcp_MailTools`(또는 `agent365` / Work IQ Mail 리소스)를 찾아 **위임(Delegated)** `McpServers.Mail.All` 추가.
4. **Grant admin consent** 클릭(테넌트 관리자 필요할 수 있음) → 이후 앱에서 **재로그인**.
5. 비밀 클라이언트면 **Certificates & secrets** 의 secret 을 `.env`의 `CLIENT_SECRET` 에,
   공개 클라이언트로 쓰려면 **Authentication → Allow public client flows** 활성화.

> 참고: 조직 정책상 해당 리소스가 "APIs my organization uses"에 안 보이면
> 관리자에게 Work IQ Mail(preview) 노출/동의를 요청해야 합니다.
> 재사용 시 `Scope 'McpServers.Mail.All' is not present` 오류가 나면 3~4번이 안 된 상태입니다.
...
1. 브라우저에서 `http://localhost:3001` → **Microsoft 로그인**.
2. 로그인되면 좌측에 Mail MCP 도구 목록이 뜹니다(연결 표시등이 초록).
3. 사용 방법:
   - **도구 직접 실행(LLM 불필요):** 좌측 도구 클릭 → arguments(JSON) 입력 → 실행.
   - **자연어 채팅(LLM 필요):** `.env`에 Azure OpenAI 또는 OpenAI 키를 넣으면
     "안 읽은 메일 검색해줘" 같은 문장으로 요청 가능.
...
Microsoft Graph 메일 도구입니다. 실제 도구 이름은 `mcp_MailTools_graph_mail_` 접두어가 붙습니다.

- **작성/전송:** createMessage(초안 생성), sendMail(즉시 전송), sendDraft(초안 전송),
  reply(회신), replyAll(전체 회신)
- **조회/검색:** getMessage(단건 조회), listSent(보낸 편지함 목록),
  searchMessages(KQL 검색 — 제목/본문/첨부)
- **관리:** updateMessage(제목/본문/카테고리/중요도 수정), deleteMessage(삭제)

> HTML 본문은 body의 `contentType = "HTML"`(지원 시 `preferHtml`)로 지정합니다.
> 업데이트/삭제는 `If-Match`(ETag) 동시성 제어를 지원합니다.
...
- **`invalid_client` / redirect 오류:** Redirect URI가 `http://localhost:3001/auth/callback`와
...
> 이 Mail MCP 서버는 **preview** 기능입니다. 도구 이름·파라미터가 변경될 수 있으니
